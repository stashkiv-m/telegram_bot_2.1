...
def process_signals(update: Update, context: CallbackContext) -> None:
    ...
    df_filtered['timestamp'] = pd.to_datetime(df_filtered['timestamp']).dt.strftime('%m/%d/%y')
    df_filtered.rename(columns={'Take Profit (%)': 'TP', 'Stop Loss (%)': 'SL', 'timestamp': 'Signal Date'}, inplace=True)
    ...
    print(f"Сигнали з Profit (%) більше 20 збережено в {output_file_txt}")
